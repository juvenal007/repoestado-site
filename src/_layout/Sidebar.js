...
const SidebarItem = ({ item, isActive }) => (
    <li className={isActive ? 'active' : ''}>
        <Link to={item.path} title={item.name}>
            ...
            {item.boton && <span className=''><Button size="lg" block className='inline font-weight-bold' color={item.boton.color}><Trans i18nKey={item.translate}>{item.boton.nombre}</Trans></Button></span>}
            {item.name && <span><Trans i18nKey={item.translate}>{item.name}</Trans></span>}
        </Link>
    </li>
